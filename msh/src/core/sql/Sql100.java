    ...
    public void initVersionSql() {
        ...
        */
        ...
        add(1, "create table system_user(id int auto_increment primary key, username varchar(255), gender bit, phone varchar(255), account varchar(255), " +
				"password varchar(255), creation_time datetime, last_login_time datetime, status int, role_id int)");

        //系统角色表
        add(2, "create table system_role(id int auto_increment primary key, name varchar(255), code varchar(255), authority varchar(255), " +
                "creation_time datetime, update_time datetime)");
		...
		//默认管理员
		add(5, "insert into system_user (username, gender, phone, account, password, creation_time, status, role_id) " +
                "values ('唐永頔', 1, '13540224775', 'tyd', '202cb962ac59075b964b07152d234b70', now(), 1, 1)",

                "insert into system_role (name, code, authority, creation_time, update_time) " +
                        "values ('管理员', 'ADMIN', 'ALL', now(), now())");
    }
}
